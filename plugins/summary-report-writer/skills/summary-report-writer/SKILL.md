...
**작업 폴더 준비 (환경별 분기)**
- 파일시스템 접근 가능 환경(Claude Code, Claude Cowork 등): `{원본파일명}/` 폴더를 생성하고, 이후 만드는 모든 과정 파일을 그 안에 저장한다. 원본 파일 자체는 옮기지 않는다.
- 파일시스템 접근 불가 환경(claude.ai 웹챗, Claude 데스크톱 앱의 Chat 탭 등): 이 단계는 건너뛰고, 대신 산출물 파일명 앞에 원본 파일명을 붙인다 (`{원본파일명}_상세파악.md` 등).
...
**호출 방식 (환경별 분기)**
- 서브에이전트 실행 가능 환경(Claude Code, 이 플러그인을 설치한 Claude Cowork): 이 점검을 직접 하지 않고 항상 `summary-report-doc-reviewer` 서브에이전트를 호출해 결과를 받는다 (정의는 D절 참고).
...
| 항목 | 내용 |
| --- | --- |
| 서브에이전트 | `summary-report-doc-reviewer` (이 플러그인의 `agents/summary-report-doc-reviewer.md`) |
...
| 환경 제약 | Claude Code, 또는 이 플러그인을 설치한 Claude Cowork에서 동작한다. 서브에이전트를 실행할 수 없는 환경(claude.ai 웹챗, Claude 데스크톱 앱의 Chat 탭 등)에서는 서브에이전트를 호출하는 대신 메인 에이전트가 같은 체크리스트를 직접 수행한다 |

검토 결과는 별도로 보고하지 않고, `{원본파일명}_보고서초안.md` 본문 가장 마지막 "자가 검토 결과" 섹션에 기록한다. 미흡해서 다시 다듬은 경우에도 다듬은 뒤의 최종 판단을 기록한다.

---

원본 표준지침: `요약보고서_작성_표준지침.md` (v43) 기준으로 패키징됨. 원본이 개정되면 이 스킬도 함께 재동기화해야 한다.
